...
def gere_key(key, xvar):
    ...
    if key == 113:  # 'q'
        ...
        for offset in range(0, xvar.img_2.sl * 100, 4):
            xvar.img_2.data[offset:offset +
                            4] = (0xFFFF0000).to_bytes(4, 'little')
    ...
    xvar.mlx.mlx_string_put(xvar.mlx_ptr, xvar.win_1,
                            20, 20, 0xFFFF00FF, "Hello MLX!")
...
def main():
    ...
    xvar.mlx.mlx_hook(xvar.win_1, 33, 0, gere_close_1,
                      xvar)  # WM_DELETE_WINDOW
    xvar.mlx.mlx_hook(xvar.win_2, 33, 0, gere_close_2,
                      xvar)  # WM_DELETE_WINDOW
