...
class MazeGenerator:
    ...
    def carve_passages(self, start_row: int, start_col: int) -> None:
        ...
        while len(stack) > 0:
            # Miramos dónde está parado el topo actualmente
            # (la última casilla de la lista)
            current = stack[-1]
            curr_row = current[0]
            curr_col = current[1]
            ...
            if len(neighbors) > 0:
                ...
                # ¡ROMPEMOS LA PARED DE LA CELDA VECINA!
                # (Tiene que ser la pared opuesta)
                if direction == "north":
                ...
                # El topo avanza: marca la nueva celda como visitada
                # y suelta hilo (la añade)
                self.cells[(next_row, next_col)].visited = True
                stack.append((next_row, next_col))
            else:
                # Si está rodeado de túneles ya excavados, recoge hilo y
                # retrocede
                stack.pop()
    ...
    def calculate_hex_for_all(self) -> None:
        ...
        while i < len(keys):
            ...
            # Calculamos la suma de las paredes que siguen en pie
            # (multiplicadas por su peso)
            valor: int = 0
